...
Go 和 Java 数据面已开始升级为生产级路由：启动时读取 `CLUSTER SLOTS` 构建 slot cache，遇到 `MOVED` 响应时更新单 slot 并按需初始化 backend 连接，并支持周期性拓扑刷新。两个数据面均使用同一 client/backend 的稳定后端连接亲和，避免依赖型 pipeline 在多后端连接间执行乱序。
